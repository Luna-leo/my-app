      ...
      for (let i = 0; i < dataPoints.length; i++) {
        ...
        if (dataPoint.data && typeof dataPoint.data === 'object') {
          ...
          if (parameterId in dataPoint.data) {
            ...
            // Log detailed information about missing parameter (only once per series)
            if (timestamps.length === 1) {
              const dataKeys = Object.keys(dataPoint.data);
              console.warn(`[transformDataForChart] Parameter "${parameterId}" not found in data for metadataId ${metadataId}`);
              ...
              if (similarKeys.length > 0) {
                console.warn(`[transformDataForChart] Found similar keys:`, similarKeys);
              }
            }
            // Push null for missing parameter values
            values.push(null);
          }
        } else {
